...
TZZ'S BLOG：个人开发者门户（首页 = Hero + 应用展示 + 博客预览），基于 **Astro 7 + TypeScript Strict + Content Collections**，部署于 GitHub Pages（GitHub Actions 自动构建）。全站代码注释、提交信息、内容均为中文。
...
- **元数据**：`src/data/apps.ts` 的 `apps` 数组（slug/name/description/status/platforms/icon/links），顺序即展示顺序。
- **详情内容**：`src/content/apps/<slug>/` 下的 `index.md`（产品介绍）/ `changelog.md` / `privacy.md`。
- **新增应用 = 两步**：`apps.ts` 追加记录 + 创建同名内容目录，路由 `/apps/<slug>[/changelog|/privacy]` 自动生成，无需注册。
- 应用详情页（`src/pages/apps/[slug]/index.astro`）用 `getCollection('apps')` 查找 `entry.id === app.slug`（Astro 7 glob loader 会把 `index.md` 的 id 归一为目录名）；正文自带 H1 会被 CSS 隐藏避免与 Hero 重复。

### 博客模块

- **schema**：`src/content.config.ts`（`blog` + `apps` 两个 zod schema，Astro 7 glob loader）。
